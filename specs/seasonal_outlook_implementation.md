...
Release check, 25 September 2026 (coherence refactor, Phase 7): the Linux image built from the committed branch (Python 3.11, clean install of `requirements.txt`) passed the full suite, **843 passed and 3 skipped**, including every Seasonal test. Seasonal snapshots taken in the image match the Windows ones except for the hashes of the synthetic maps, which the image encodes differently. `terraform validate` passes on `deploy/seasonal-outlook/main.tf` (Terraform 1.9.8, Google provider 7.46.1). A `terraform plan` and the release gate above remain to be done in the company project.

...
class Settings:
    ...
    def from_env(cls):
        fields = {k: os.environ['SEASONAL_' + k.upper()] for k in
                  ('project', 'bucket', 'database', 'collection', 'prefix', 'model', 'location', 'signer', 'backend')
                  if 'SEASONAL_' + k.upper() in os.environ}
        return cls(enabled=os.getenv('SEASONAL_DRAFTER_ENABLED', 'true').strip().lower() == 'true', **fields)

    def errors(self):
        errors = [f'SEASONAL_{k.upper()} is required' for k in ('project', 'bucket', 'signer') if not getattr(self, k)]
